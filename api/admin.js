...
// jeremy: importing all db functions from index
...
async function requireAdmin(req, res, next) {
  ...
  if (checkAdmin.isadmin === true) {
    ...
    res.send({
      ...
      message: "Please insert correct Credentials and try again",
    });
    // jeremy:
...
adminRouter.get("/getUsers", requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!!allUsers) {
      res.send({
        name: "FetchingAllUsers",
        message: "Successfully Caight users",
        status: "successful",
        users: allUsers,
      });
    } else {
      res.send({
        name: "FetchProblem",
        message: "Problem with fetching users",
      });
    }
  } catch (error) {
    console.log(error);
...
adminRouter.post("/:user", requireAdmin, async (req, res, next) => {
  try {
    ...
    if (checkUser) {
      ...
      res.send({
        name: "PromoteAdmin",
        message: `you have successfully promoted userId ${id} to admin`,
      });
    } else {
      res.send({
        name: "NoUser",
        message: "There is not a user with that name",
      });
    }
  } catch (error) {
    console.log(error);
...
adminRouter.post("/createProduct", requireAdmin, async (req, res, next) => {
  try {
    ...
    // jeremy: send back a success of create
  } catch (error) {
...
adminRouter.post("/update/:itemId", requireAdmin, async (req, res, next) => {
  try {
    // jeremy: send back a success of update
  } catch (error) {
    // handle the catch error
  }
});
